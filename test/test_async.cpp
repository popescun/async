...
public:
  ...
  {
    std::cout << "A::f_with_arg_2 " << x << " " << y << " thread " << std::this_thread::get_id() << std::endl;
  }

  int f_with_arg_and_return(int x)
  {
    std::cout << "A::f_with_arg_and_return " << x << " thread " << std::this_thread::get_id() << std::endl;
    return x;
  }
...
int main()
{
  ...
  untangle::async::execution<std::function<void(void)>> asyncexec;
  asyncexec.name = "asyncexec"; //debug
  ...
  untangle::async::execution<std::function<void(int)>> asyncexec1;
  asyncexec1.name = "asyncexec1";
  asyncexec1.bind_action_and_method(a->async_f_with_arg, a, &A::f_with_arg);
  asyncexec.attach(asyncexec1);
  a->async_f_with_arg(10);

  untangle::async::execution<std::function<void(int, int)>> asyncexec2;
  asyncexec2.name = "asyncexec2";
  ...
  untangle::async::execution<std::function<int(int)>> asyncexec3;
  asyncexec3.name = "asyncexec3";
  ...
  untangle::async::execution<std::function<void(void)>> asyncexec5;
  asyncexec5.name = "asyncexec5";
